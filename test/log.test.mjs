...
function loadCommands(initial) {
  ...
  sb.GT = {
    ...
    skin: { hide() {}, current: { system: (lvl, x, node) => { said.push(lvl + ':' + x); if (node) nodes.push(node); },
...
// --- :log dump · clear ---
{
  ...
  t('화면의 진단 줄도 걷어낸다', a.GT.skin.current.screen === 0);
  ...
  t('명령이 그걸 부른다', /GT\.skin\.current\.clearSystem\(\)/.test(fs.readFileSync('src/content/commands.js', 'utf8')));
