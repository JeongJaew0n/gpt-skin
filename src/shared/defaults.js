...
//
// skin: '<id>' (또는 ['<id>', …]) 가 붙은 항목은 그 스킨에서만 뜻이 있다. 설정 화면과 :config 는 지금 스킨의
...
var GT_SCHEMA = [
  ...
  // none 은 대화를 그리지 않는다.
  { section: 'display', key: 'citations', type: 'enum', def: 'domain', skin: ['terminal', 'sheet'],
    choices: ['domain', 'number', 'off'] },
  // 그림을 셀·명령줄에 그리지 않는다 — 터미널만 그린다.
  { section: 'display', key: 'image', type: 'enum', def: 'inline', skin: 'terminal',
    choices: ['inline', 'blocks', 'off'] },
  { section: 'display', key: 'image.columns', type: 'int', def: 48, min: 16, max: 160, skin: 'terminal' },
...
// 이 항목이 그 스킨에서 뜻이 있는가. skin 표시가 없으면 공용이다.
var GT_SKIN_HAS = function (f, skin) {
  if (!f.skin) return true;
  return Array.isArray(f.skin) ? f.skin.indexOf(skin) >= 0 : f.skin === skin;
};

// 이 스킨에서 보여 줄 항목 — 공용 + 그 스킨의 것.
var GT_FIELDS_FOR = function (skin) {
  return GT_SCHEMA.filter(function (f) { return GT_SKIN_HAS(f, skin); });
};
