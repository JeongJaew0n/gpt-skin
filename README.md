...
![테스트 1448](https://img.shields.io/badge/테스트-1448_케이스-22C55E)
...
맨 위에서 **스킨**(터미널 · 시트 · 노 스킨)을 고른다. 고르면 저장되고 열린 ChatGPT 탭이 바로 바뀐다.

| 토글 | 무엇을 |
|---|---|
| **이 탭에 스킨 켜기** | 지금 보고 있는 탭에만. 저장하지 않는다 |
| **ChatGPT 를 열면 바로 스킨으로** | 기본 동작. `enabled` 로 저장된다 |
...
<details>
<summary><b>40개 파일 · 1448 케이스</b></summary>
...
| `skinconfig` | 39 | 스킨 설정 — skin 선택지 = 스킨 파일 · 스킨 전용 항목 필터 · theme 이관이 한 번만, 사용자 값을 덮지 않는가 · `:skin` `:theme` |
